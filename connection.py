...
class Connection:
    ...
    def draw(self, screen, build_pos):
        ...
        if self.connected_to and self.type == EConnectionType.OUTLET:
            start_pos = [self.build.pos[0] + self.pos[0] + CONNECTION_SIZE // 2, self.build.pos[1] + self.pos[1] + CONNECTION_SIZE // 2]
            end_pos = [self.connected_to.build.pos[0] + self.connected_to.pos[0] + CONNECTION_SIZE // 2, self.connected_to.build.pos[1] + self.connected_to.pos[1] + CONNECTION_SIZE // 2]
            pygame.draw.line(screen, CONNECTION_LINE_COLOR, start_pos, end_pos, CONNECTION_LINE_SIZE)
